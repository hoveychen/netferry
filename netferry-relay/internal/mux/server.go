...
type chanState struct {
	inbox chan Frame // frames from mux destined for the downstream conn
	once  sync.Once // ensures close(inbox) is called once
}
...
func (s *MuxServer) CloseChannel(channel uint16) {
	...
	s.mu.Unlock()
	if ok {
		cs.once.Do(func() { close(cs.inbox) })
	}
}
...
func (s *MuxServer) dispatch(f Frame) error {
	...
	default:
		...
		if ok {
			...
			case cs.inbox <- f:
			default:
				// Inbox full — drop frame. Go's natural backpressure means
				// the sender will slow down before this happens in normal use.
			}
		}
	}
	return nil
...
func (s *MuxServer) HandleTCP(channel uint16, family int, dstIP string, dstPort int) {
	...
	go func() {
		...
		for {
			n, err := conn.Read(buf)
			if n > 0 {
				// Copy slice before sending — the buffer is reused.
				d := make([]byte, n)
				copy(d, buf[:n])
